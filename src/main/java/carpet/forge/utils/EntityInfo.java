...
public class EntityInfo {
    ...
    public static List<String> entityInfo(Entity e, World ws) {
        ...
        if (e.isRiding()) {
            lst.add(String.format(" - Rides: %s", e.getRidingEntity().getDisplayName().getUnformattedText()));
        }
        if (e.isBeingRidden()) {
        ...
        if (((IMixinEntity) e).getFire() > 0) {
            lst.add(String.format(" - Fire for %d ticks", ((IMixinEntity) e).getFire()));
        }
        if (e.isImmuneToFire()) {
            lst.add(" - Immune to fire");
        }
        if (e.timeUntilPortal > 0) {
            lst.add(String.format(" - Portal cooldown for %d ticks", e.timeUntilPortal));
        }
