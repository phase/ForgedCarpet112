...
public class SpawnReporter {
    ...
    public static void registerSpawn(EntityLiving el, String type, String mob, BlockPos pos) {
        registerSpawn(el, type, mob, pos, 1L);
    }
            ...
            if (!worldIn.isRemote) {
                ...
                if (lst != null && !lst.isEmpty()) {
                    for (Biome.SpawnListEntry animal : lst) {
                        ...
                        int weight = ((IMixinBiome) animal).getWeight();
                        if (canspawn) {
